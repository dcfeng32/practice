...
public class gongzi_servlet extends HttpServlet {
	public void service(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		if (type.endsWith("lishi")) {
			gongziHistory(req, res);
		}
	}

	public void gongziHistory(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		
		TZhigong zhigong = (TZhigong) req.getSession().getAttribute("user");
		String sql = "select * from t_history where zhigong_id=" + zhigong.getId();
		req.setAttribute("history", getGongziList1(sql));
		req.getRequestDispatcher("admin/gongzi/gongziHis.jsp").forward(req, res);
	...
	public void gongziAdd(HttpServletRequest req, HttpServletResponse res) {
		...
		String sql = "insert into t_gongzi (zhigong_id,jiben,gongling,zhiwu,butie) " + "values(?,?,?,?,?)";
	...
	private List getGongziList1(String sql) {
		List gongziList = new ArrayList();
		Object[] params = {};
		DB mydb = new DB();
		try {
